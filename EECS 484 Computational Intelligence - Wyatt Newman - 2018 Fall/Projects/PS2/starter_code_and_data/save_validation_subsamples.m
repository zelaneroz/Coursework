function save_validation_subsamples(fname_root, n_subimages,n_pixel_samps)
...
%single garment
%output the results in terms of overly-large feature vectors,
%one vector per sampled image
ndivs = 8;
...
[ color_samps1, scaled_color_samps, large_cubes_matrix ] = sample_colors_from_image( A,n_pixel_samps, ndivs );
for ipic=1:n_subimages-1
    [ color_samps1, scaled_color_samps, cubes_vec ] = sample_colors_from_image( A,n_pixel_samps, ndivs );
...
fname_save = strcat(fname_root,'_cubes_matrix_v')
save(fname_save,'large_cubes_matrix')
fname_save = strcat(fname_root,'_net_vec_v')
