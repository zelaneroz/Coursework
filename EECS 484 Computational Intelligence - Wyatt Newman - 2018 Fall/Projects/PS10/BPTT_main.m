%PS10 BPTT, recurrent neural net
...
clear all
Nneurons=8; %4; %CHOOSE THIS; N=4 for  single "interneuron" case
INIT_WEIGHT_MAG=1; %scale synapse initializations to random number +/- in this range
...
while rms_error > RMS_THRESH
    ...
    for t=T_time_steps:-1:1
        F_sigmas(:,t)=compute_F_sigma(W,errs,F_uvals,t); %F_sigma at time t first,
    ...
    [F_wji]=compute_F_wji(sigma_history,F_uvals)
    ...
    rms_error=sqrt(Esqd/T_time_steps)
%     if mod(curr_iter,300)==0
%         figure(1)
...
pause(1)
fprintf('BPTT took %d iterations to converge\n', curr_iter)