...
def data_input(if_random = True, if_training = True):
    if(if_training):
        filenames = [os.path.join(DATA_DIR, "train-0000%d-of-00002.tfrecord" % i) for i in xrange(0, 1)]
    else:
        filenames = [os.path.join(DATA_DIR, "validation-0000%d-of-00002.tfrecord" % i) for i in xrange(0, 1)]
...
def net_inference(image_batch):
    ...
    depth3 = 96 #the depth of 2nd convnet
    C5_units = 128
    F6_units = 64
    F7_units = 7
...
def net_train():
    image_batch_out, label_batch_out, filename_batch = data_input(if_random = False, if_training = True)

    image_batch_placeholder = tf.placeholder(tf.float32, shape=[BATCH_SIZE, IMAGE_SIZE, IMAGE_SIZE, CHANNELS])
    image_batch = tf.nn.dropout(tf.reshape(image_batch_out, (BATCH_SIZE, IMAGE_SIZE, IMAGE_SIZE, CHANNELS)),0.8)

    label_batch_placeholder = tf.placeholder(tf.float32, shape=[BATCH_SIZE, 7])
    label_offset = -tf.ones([BATCH_SIZE], dtype=tf.int64, name="label_batch_offset")
    label_batch_one_hot = tf.one_hot(tf.add(label_batch_out, label_offset), depth=7, on_value=1.0, off_value=0.0)
...
def net_eval():
    image_batch_out, label_batch_out, filename_batch = data_input(if_random = False, if_training = False)
    ...
    logits_out = tf.reshape(net_inference(image_batch_placeholder), [BATCH_SIZE, 7])
